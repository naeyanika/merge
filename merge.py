...
if 'pinjaman_na' in dfs:
    ...
    def sum_lists(x):
        ...
        return x 
            ...
            pivot_table1.columns = [f'{col[0]}_{col[1]}' for col in pivot_table4.columns]
            pivot_table1.reset_index(inplace=True)
            pivot_table1['TRANS. DATE'] = pd.to_datetime(pivot_table4['TRANS. DATE'], format='%d%m%Y').dt.strftime('%d/%m/%Y')
